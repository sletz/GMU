...
{
    t_class* c = class_new("idleubutton", (method)idleubutton_new, (method)idleubutton_free,
                           sizeof(t_idleubutton), 0L, A_GIMME, 0);
...
void* idleubutton_new(t_symbol* s, long argc, t_atom* argv)
{
    ...
    boxflags = 0 | JBOX_DRAWFIRSTIN | JBOX_NODRAWBOX | JBOX_DRAWINLAST | JBOX_TRANSPARENT |
               JBOX_GROWBOTH | JBOX_DRAWBACKGROUND;
...
void idleubutton_paint(t_idleubutton* x, t_object* patcherview)
{
    t_rect       rect;
    t_jgraphics* g =
        (t_jgraphics*)patcherview_get_jgraphics(patcherview);  // obtain graphics context
...
void inside(t_idleubutton* x, t_pt pt, t_object* patcherview)
{
    if (patcherview_get_presentation(patcherview)) {
        if (pt.x <= 0 || pt.y <= 0 || pt.x >= x->b_box.b_presentation_rect.width ||
            pt.y >= x->b_box.b_presentation_rect.height) {
            x->b_inside = 0;
        } else {
            x->b_inside = 1;
        }
    } else {
        if (pt.x <= 0 || pt.y <= 0 || pt.x >= x->b_box.b_patching_rect.width ||
            pt.y >= x->b_box.b_patching_rect.height) {
            x->b_inside = 0;
        } else {
            x->b_inside = 1;
        }
