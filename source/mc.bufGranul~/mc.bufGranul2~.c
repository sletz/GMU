...
void ext_main(void* r)
{
    ...
    c = class_new("mc.bufGranul~", (method)bufGranul_new, (method)bufGranul_free,
                  sizeof(t_bufGranul), (method)NULL, A_GIMME, 0L);
                       ...
                       long numouts, long sampleframes, long flags, void* userparam)
{
    ...
    int t_active_buf;  // buffer actif pour la cr�ation si pas entr�e bufnum signal (
                       // x->x_active_buf peut ne plus �tre valide )
    int    t_bufnum;
    int    t_bufchan;
    ...
    perform:
        ...
        for (i = 0; i < NVOICES; i++) {
            ...
            if (x->x_voiceOn[i] > 0)  //&& x->x_ind[i] < x->Vlength[i] )
            {
                ...
                // grains) Il se peut que le buffer associ� au grain ne soit plus valide ->
