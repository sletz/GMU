...
// BUFFER ENVELOPPE
    ...
    int x_active_env;  // le buffer enveloppe actif est celui dans lequel les nouveaux grains sont
                       // prélevés.
    ...
    double *Vfreq, *Vphase, *Vamp, *Vlength, *Vpan, *Vdist;
    double* Vphase_inc;  // pas d'increment de la phase du sinus

    double *Vmem1,
        *Vmem2;  // memoires internes des oscilateurs ( fast sine calculation MAGIC CIRCLE )
    double* Vcoef;
...
// Instanciation
void* synGranul_new(t_symbol* s, short ac,
...
// Buffer son
void synGranul_set(t_synGranul* x, t_symbol* msg, short ac,
                   t_atom* av);  // definition des buffer~ son
// void synGranul_setInit(t_synGranul *x, t_symbol *s);	// definition du buffer~ son (à
// l'initialisation du patch)
...
// Bufffer enveloppe
void synGranul_setenv(t_synGranul* x, t_symbol* msg, short ac,
...
void synGranul_grain(t_synGranul* x, t_symbol* s, short ac,
                     t_atom* av);  // declenchement d'un grain par liste
void synGranul_float(t_synGranul* x,
...
void synGranul_assist(t_synGranul* x, void* b, long m, long a,
...
#else
void synGranul_dsp64(t_synGranul* x, t_object* dsp64, short* count, double samplerate,
                     long maxvectorsize, long flags);  // signal processing
#endif
void synGranul_free(t_synGranul* x);  // liberation de la memoire
void synGranul_perform64(t_synGranul* x, t_object* dsp64, double** ins, long numins, double** outs,
...
{
    synGranul_class =
        class_new(gensym("synGranul~"), (t_newmethod)synGranul_new, (t_method)synGranul_free,
                  (short)sizeof(t_synGranul), 0L, A_GIMME, 0);

    CLASS_MAINSIGNALIN(synGranul_class, t_synGranul, x_f);

    class_addmethod(synGranul_class, (t_method)synGranul_setenv, gensym("setenv"), A_GIMME,
                    0);  // definition du buffer~ enveloppe
    class_addmethod(synGranul_class, (t_method)synGranul_envbuffer, gensym("envbuffer"), A_FLOAT,
                    0);  // # env buffer actif

    class_addmethod(synGranul_class, (t_method)synGranul_nvoices, gensym("nvoices"), A_FLOAT,
                    0);  // nombre de voix (polyphonie)
    class_addmethod(synGranul_class, (t_method)synGranul_tellme, gensym("tellme"),
                    0);  // demande d'info
    class_addbang(synGranul_class,
                  (t_method)synGranul_bang);  // routine pour un bang, declenchement d'un grain
#ifndef ARCH_PD
    addfloat((t_method)synGranul_float);  // affectation des valeurs par des flottants
#endif

    // class_addmethod((t_method)synGranul_assist, gensym("assist"), A_CANT, 0);		//
    // assistance in out
    class_addmethod(synGranul_class, (t_method)synGranul_dsp, gensym("dsp"),
                    0);  // signal processing
    class_addmethod(synGranul_class, (t_method)synGranul_sinterp, gensym("sinterp"), A_FLOAT,
                    0);  // interpollation dans la lecture du buffer pour éviter les clics
    class_addmethod(synGranul_class, (t_method)synGranul_clear, gensym("clear"),
                    0);  // panique ! effacement des grains en cours
    class_addmethod(synGranul_class, (t_method)synGranul_clear, gensym("panic"),
                    0);  // panique ! effacement des grains en cours
...
{
    t_class* c = class_new("synGranul~", (method)synGranul_new, (method)synGranul_free,
                           sizeof(t_synGranul), (method)NULL, A_GIMME, 0L);

    class_addmethod(c, (method)synGranul_setenv, "setenv", A_GIMME,
                    0);  // definition du buffer~ enveloppe
    class_addmethod(c, (method)synGranul_envbuffer, "envbuffer", A_LONG, 0);  // # env buffer actif

    class_addmethod(c, (method)synGranul_nvoices, "nvoices", A_LONG,
                    0);                                         // nombre de voix (polyphonie)
    class_addmethod(c, (method)synGranul_tellme, "tellme", 0);  // demande d'info
    class_addmethod(c, (method)synGranul_bang, "bang",
                    0);  // routine pour un bang, declenchement d'un grain
    class_addmethod(c, (method)synGranul_float, "float", A_FLOAT,
    ...
    class_addmethod(c, (method)synGranul_sinterp, "sinterp", A_LONG,
                    0);  // interpollation dans la lecture du buffer pour éviter les clics
    class_addmethod(c, (method)synGranul_clear, "clear",
                    0);  // panique ! effacement des grains en cours
    class_addmethod(c, (method)synGranul_clear, "panic",
                    0);  // panique ! effacement des grains en cours
...
void synGranul_grain(t_synGranul* x, t_symbol* s, short ac, t_atom* av)
{
    ...
    if (ac < 8) {
        post(
            "syngranul~ : grain args are <delay(ms)> <freq> <phase> <amp> <length> <pan> <dist> "
            "<envbuffer>");
        return;
    }
        ...
        case 1:  // inlet
            switch (a) {
                case 0:
                    ...
                    break;
                case 1:
                    sprintf(s, " freq (float & signal) ");
                    break;
                case 2:
                    sprintf(s, " phase (float & signal) ");
                    break;
                case 3:
        ...
        while (n-- && p) {
            ...
            if (prevsigin <= 0 && sigin > 0)  // creation d'un grain
            {
                ...
                if (p) {
                    ...
                    x->x_voiceOn[p] = 1;  // activation
                    x->Vfreq[p] =
                        (x->x_in2con) * t_freq[xn] + (1 - x->x_in2con) * x->x_freq;  // frequence
                    ...
                    if (x->x_in5con) {
                        if (t_length[xn] < 0) {
                            ...
                            x->envinc[p] =
                                -1. * (float)(x->x_env_frames[t_active_env] - 1.) / x->Vlength[p];
                            x->envind[p] = x->x_env_frames[t_active_env] - 1;
                        } else {
                            x->Vlength[p] = t_length[xn] * srms;
                            x->envinc[p] =
                                (float)(x->x_env_frames[t_active_env] - 1.) / x->Vlength[p];
                            x->envind[p] = 0.;
                        }
                    } else {
    ...
    for (cpt = 0; cpt < x->x_nenvbuffer; cpt++) {
        if (x->x_env_sym[cpt])  // si un nom de buffer a ŽtŽ donnŽ -> collecter les infos
        {
            if (!(a = (t_garray*)pd_findbyclass(x->x_env_sym[cpt], garray_class))) {
                if (x->x_env_sym[cpt]->s_name) {
                    pd_error(x, "synGranul~: %s: no such array", x->x_env_sym[cpt]->s_name);
                }
                x->x_env_buf[cpt] = 0;
                if (cpt == 0) {
                    return 0;
    ...
    for (cpt = 0; cpt < x->x_nenvbuffer; cpt++) {
        if (x->x_env_sym[cpt])  // si un nom de buffer a ŽtŽ donnŽ -> collecter les infos
        {
                ...
                b->b_valid && !b->b_inuse)  // attention inuse !!!
            {
                ...
                // post("invalid %d",cpt);
                //  le buffer 0 est celui utilisŽ si celui choisi est pas valide ... si il n'y est
                //  pas -> pas dsp !!!
                if (cpt == 0) {
                    return 0;
                }
...
void synGranul_setenv(t_synGranul* x, t_symbol* msg, short ac, t_atom* av)
{
    ...
    if (!(a = (t_garray*)pd_findbyclass(x->x_env_sym[num], garray_class))) {
        if (*s->s_name) {
            pd_error(x, "syngranul~: %s: no such array", x->x_env_sym[num]->s_name);
        }
        x->x_env_buf[num] = 0;
...
void synGranul_dsp(t_synGranul* x, t_signal** sp)
{
    ...
    x->x_in2con = x->x_in3con = x->x_in4con = x->x_in5con = x->x_in6con = x->x_in7con =
        x->x_in8con                                                     = 1;

    switch (n) {
        case 1:
            dsp_add(synGranul_perform1, 10, x, sp[0]->s_vec, sp[1]->s_vec, sp[2]->s_vec,
                    sp[3]->s_vec, sp[4]->s_vec, sp[5]->s_vec, sp[6]->s_vec, sp[7]->s_vec,
                    sp[0]->s_n);
            break;

        case 2:
            dsp_add(synGranul_perform2, 11, x, sp[0]->s_vec, sp[1]->s_vec, sp[2]->s_vec,
                    sp[3]->s_vec, sp[4]->s_vec, sp[5]->s_vec, sp[6]->s_vec, sp[7]->s_vec,
                    sp[8]->s_vec, sp[0]->s_n);
            break;

        case 4:
            dsp_add(synGranul_perform4, 13, x, sp[0]->s_vec, sp[1]->s_vec, sp[2]->s_vec,
                    sp[3]->s_vec, sp[4]->s_vec, sp[5]->s_vec, sp[6]->s_vec, sp[7]->s_vec,
                    sp[8]->s_vec, sp[9]->s_vec, sp[10]->s_vec, sp[0]->s_n);
            break;

        case 6:
            dsp_add(synGranul_perform6, 15, x, sp[0]->s_vec, sp[1]->s_vec, sp[2]->s_vec,
                    sp[3]->s_vec, sp[4]->s_vec, sp[5]->s_vec, sp[6]->s_vec, sp[7]->s_vec,
                    sp[8]->s_vec, sp[9]->s_vec, sp[10]->s_vec, sp[11]->s_vec, sp[12]->s_vec,
                    sp[0]->s_n);
            break;

        case 8:
            dsp_add(synGranul_perform8, 17, x, sp[0]->s_vec, sp[1]->s_vec, sp[2]->s_vec,
                    sp[3]->s_vec, sp[4]->s_vec, sp[5]->s_vec, sp[6]->s_vec, sp[7]->s_vec,
                    sp[8]->s_vec, sp[9]->s_vec, sp[10]->s_vec, sp[11]->s_vec, sp[12]->s_vec,
                    sp[13]->s_vec, sp[14]->s_vec, sp[0]->s_n);
            break;
    }
}
#else  // MAX
...
int synGranul_desalloc(t_synGranul* x)
{
...
#ifndef ARCH_PD
    if (x->x_ind) {
        t_freebytes(x->x_ind, NVOICES * sizeof(long));
    }
    if (x->x_remain_ind) {
        t_freebytes(x->x_remain_ind, NVOICES * sizeof(long));
    }

    if (x->Vfreq) {
        t_freebytes(x->Vfreq, NVOICES * sizeof(double));
    }
    if (x->Vphase) {
        t_freebytes(x->Vphase, NVOICES * sizeof(double));
    }
    if (x->Vamp) {
        t_freebytes(x->Vamp, NVOICES * sizeof(double));
    }
    if (x->Vlength) {
        t_freebytes(x->Vlength, NVOICES * sizeof(double));
    }
    if (x->Vpan) {
        t_freebytes(x->Vpan, NVOICES * sizeof(double));
    }
    if (x->Vdist) {
        t_freebytes(x->Vdist, NVOICES * sizeof(double));
    }

    if (x->Vphase_inc) {
        t_freebytes(x->Vphase_inc, NVOICES * sizeof(double));
    }

    if (x->Vmem1) {
        t_freebytes(x->Vmem1, NVOICES * sizeof(double));
    }
    if (x->Vmem2) {
        t_freebytes(x->Vmem2, NVOICES * sizeof(double));
    }

    if (x->Vcoef) {
        sysmem_freeptr(x->Vcoef);
    }

    if (x->envinc) {
        t_freebytes(x->envinc, NVOICES * sizeof(double));
    }
    if (x->envind) {
        t_freebytes(x->envind, NVOICES * sizeof(double));
    }
    if (x->x_env) {
        t_freebytes(x->x_env, NVOICES * sizeof(double));
    }
    if (x->Venv) {
        t_freebytes(x->Venv, NVOICES * sizeof(int));
    }

    if (x->x_delay) {
        t_freebytes(x->x_delay, NVOICES * sizeof(int));
    }
    if (x->x_voiceOn) {
        t_freebytes(x->x_voiceOn, NVOICES * sizeof(int));
    }
    ...
    if (x->Vfreq) {
        freebytes(x->Vfreq, NVOICES * sizeof(float));
    }
    if (x->Vphase) {
        freebytes(x->Vphase, NVOICES * sizeof(float));
    }
    if (x->Vamp) {
    ...
    if (x->Vphase_inc) {
        freebytes(x->Vphase_inc, NVOICES * sizeof(float));
    }

    if (x->Vmem1) {
        freebytes(x->Vmem1, NVOICES * sizeof(float));
    }
    if (x->Vmem2) {
        freebytes(x->Vmem2, NVOICES * sizeof(float));
    }

    if (x->Vcoef) {
        freebytes(x->Vcoef, NVOICES * sizeof(float));
    }
