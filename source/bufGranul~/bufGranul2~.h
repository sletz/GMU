...
typedef struct _bufGranul {
    ...
    int       x_active_buf;  // le buffer son actif est celui dans lequel les nouveaux grains sont
                             // prélevés.
                             // BUFFER SON LOOP
    ...
    long*   Vloop;
    double *Vloopstart,
        *Vloopend;  // loop start and length per voice ( Vloop == 2 means entire buffer)

    long* Vbuf;  // numero du buffer son dans lequel sera pris le grain

    int x_env_dir;  // direction de lecture de l'enveloppe
    double*
        envinc;  // pas d'avancement dans le buffer enveloppe (par rapport ˆ la longueur du grain
    double* envind;  // indice de dŽpart dans le buffer enveloppe (debut si lecture normale, fin si
                     // lecture inversŽe)
...
// Instanciation
void* bufGranul_new(t_symbol* s, short ac,
...
// Buffer son
void bufGranul_set(t_bufGranul* x, t_symbol* msg, short ac,
                   t_atom* av);  // definition des buffer~ son
// void bufGranul_setInit(t_bufGranul *x, t_symbol *s);	// definition du buffer~ son (à
...
// Bufffer enveloppe
void bufGranul_setenv(t_bufGranul* x, t_symbol* msg, short ac,
...
void bufGranul_grain(t_bufGranul* x, t_symbol* s, short ac,
                     t_atom* av);  // declenchement d'un grain par liste
void bufGranul_float(t_bufGranul* x,
...
void bufGranul_loop(t_bufGranul* x, t_symbol* s, short ac,
