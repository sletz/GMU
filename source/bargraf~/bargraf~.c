...
// inline t_jrgba RGBA(double r, double g, double b, double a) { t_jrgba v; v.red = r; v.green = g;
// v.blue = b; v.alpha = a; return v; }
...
void bargraf_dsp64(t_bargraf* x, t_object* dsp64, short* count, double samplerate,
                   long maxvectorsize, long flags);
void bargraf_perform64(t_bargraf* x, t_object* dsp64, double** ins, long numins, double** outs,
...
void ext_main(void* r)
{
    ...
    c = class_new("bargraf~", (method)bargraf_new, (method)bargraf_free, sizeof(t_bargraf),
    ...
    class_addmethod(c, (method)bargraf_saturation, "mode", A_LONG,
    ...
    class_addmethod(c, (method)bargraf_mouseenter, "mouseenter", A_CANT,
                    0);  // mouseenter on object
    class_addmethod(c, (method)bargraf_mouseleave, "mouseleave", A_CANT,
                    0);  // mouseleave on object
    ...
    CLASS_ATTR_ENUMINDEX(
        c, "drawmode", 0,
        "Standard \"Draw when mouse is over\" \"Calculate and draw when mouse is over\"");
    ...
    CLASS_ATTR_ENUMINDEX(c, "show_value", 0,
                         "Off \"Current dB value\" \"Current linear value\" \"Local dB peaks\" "
                         "\"Maximum dB value\"");
    ...
    // COLORS
    // Background, Foreground, 1. Cold, 2. Mild, 3. Warm, 4. Warning (Hot), 5. Overload, Highlighted
    // Shadow, Dark Shadow, Highlighted Axes, Dark Axes, Frame Background Color
...
t_max_err bargraf_stdargs(t_dictionary* d, t_symbol* s, long argc, t_atom* argv)
{
    ...
    // left / top / width / height / interval / mode / 3*brgb / 3*frgb / 3*c1 / 3*c2 / 3*c3 / 3*c4 /
    // 3*c5 / 3*hisha / 3*dasha / 3*hiaxe / 3*daaxe / 3*bframe / 3*countmax / 3*clockstate
    ...
    switch (m) {
        case ASSIST_INLET:
            switch (a) {
                case 0:
                    sprintf(s,
                            "(Signal) Signal to draw, (int/float) Delay between ticks (ms), clear, "
                            "bang");
                    break;
                case 1:
                    sprintf(s, "(int/float) Delay between ticks (ms)");
                    break;
            }
            break;
        case ASSIST_OUTLET:
            switch (a) {
                case 0:
                    sprintf(s, "(float) Amplitude (dB)");
                    break;
                case 1:
                    sprintf(s, "(float) Amplitude (Linear)");
                    break;
                case 2:
                    sprintf(s, "(float) Peak Amplitude (dB)");
                    break;
            }
            break;
    }
...
t_bargraf* bargraf_new(t_symbol* s, long argc, t_atom* argv)
{
    ...
    flags = 0 | JBOX_DRAWFIRSTIN | JBOX_NODRAWBOX | JBOX_DRAWINLAST |
            JBOX_TRANSPARENT
            //		| JBOX_NOGROW
            //		| JBOX_GROWY
            | JBOX_GROWBOTH |
...
void bargraf_paint(t_bargraf* x, t_object* view)
{
    ...
    if (x->b_sens) {
        if (x->b_showv) {
            ...
            jgraphics_rectangle(g, wrL + pad_value_rectw, hT + hBG + hB - pad_value_recth,
                                wBG - 2 * pad_value_rectw, hFG);
            jgraphics_set_source_jrgba(g, &x->b_frgb);
            jgraphics_fill(g);

            if (x->b_showb) {
                // contour value
                jgraphics_rectangle(g, weight - pad_value_rectw - wrR,
                                    hT + hBG + hB - pad_value_recth - penH, penW, hFG + 2 * penH);
                jgraphics_rectangle(g, wrL + pad_value_rectw, height - pad_value_recth,
                                    wBG - 2 * pad_value_rectw, penH);
                jgraphics_set_source_jrgba(g, &x->b_hisha);
                jgraphics_fill(g);

                jgraphics_rectangle(g, wrL + pad_value_rectw - penW,
                                    hT + hBG + hB - pad_value_recth - penH, penW, hFG + 2 * penH);
                jgraphics_rectangle(g, wrL + pad_value_rectw,
                                    hT + hBG + hB - pad_value_recth - penH,
                                    wBG - 2 * pad_value_rectw, penH);
                jgraphics_set_source_jrgba(g, &x->b_dasha);
                jgraphics_fill(g);
            }
            ...
            jf  = jfont_create(jbox_get_fontname((t_object*)x)->s_name,
                               jbox_get_font_slant((t_object*)x), jbox_get_font_weight((t_object*)x),
            ...
            //			else
            //				sprintf(text,"%.2f",x->b_maxvaldB);
            jtextlayout_set(jtl, text, jf, wrL + pad_value_font,
                            hT + hBG + hB - pad_value_recth + pad_value_font,
                            wBG - 2 * pad_value_font, hFG - 2 * pad_value_font,
                            JGRAPHICS_TEXT_JUSTIFICATION_CENTERED, JGRAPHICS_TEXTLAYOUT_NOWRAP);
            ...
            jtl = jtextlayout_create();
            sprintf(text, "~");
            jtextlayout_set(jtl, text, jf, wrL, hT + hBG, wBG, hB,
                            JGRAPHICS_TEXT_JUSTIFICATION_CENTERED, JGRAPHICS_TEXTLAYOUT_NOWRAP);
