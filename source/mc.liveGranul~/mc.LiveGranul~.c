...
typedef struct _liveGranul {
    ...
    int x_loop;  // loop on/off (0 : no loops , 1 : loop entire buffer, 2 : loop between loopstart &
                 // end )
    double x_loopstart;
    double x_loopend;
...
// Instanciation
void* liveGranul_new(t_symbol* s, short ac,
...
// Buffer son
void liveGranul_set(t_liveGranul* x, t_symbol* msg, short ac,
                    t_atom* av);  // definition des buffer~ son
// void liveGranul_setInit(t_liveGranul *x, t_symbol *s);	// definition du buffer~ son (�
...
// Bufffer enveloppe
void liveGranul_setenv(t_liveGranul* x, t_symbol* msg, short ac,
...
void liveGranul_float(t_liveGranul* x,
...
// void liveGranul_dist(t_liveGranul *x, double f);			// modification de la distance (pour le
// pan)

void liveGranul_assist(t_liveGranul* x, void* b, long m, long a,
                       char* s);  // assistance info inlet, out1et

// DSP
void liveGranul_free(t_liveGranul* x);  // liberation de la memoire
void liveGranul_dsp64(t_liveGranul* x, t_object* dsp64, short* count, double samplerate,
                      long maxvectorsize, long flags);  // signal processing
void liveGranul_perform64(t_liveGranul* x, t_object* dsp64, double** ins, long numins,
...
{
    t_class* c = class_new("mc.liveGranul~", (method)liveGranul_new, (method)liveGranul_free,
                           sizeof(t_liveGranul), (method)NULL, A_GIMME, 0L);

    class_addmethod(c, (method)liveGranul_setenv, "setenv", A_GIMME,
    ...
    class_addmethod(c, (method)liveGranul_nvoices, "nvoices", A_LONG,
                    0);                                          // nombre de voix (polyphonie)
    class_addmethod(c, (method)liveGranul_tellme, "tellme", 0);  // demande d'info
    class_addmethod(c, (method)liveGranul_bang, "bang",
                    0);  // routine pour un bang, declenchement d'un grain
    class_addmethod(c, (method)liveGranul_grain, "grain", A_GIMME,
                    0);  // d�clenchement des grains par message
    class_addmethod(c, (method)liveGranul_float, "float", A_FLOAT,
    ...
    class_addmethod(c, (method)liveGranul_sinterp, "sinterp", A_LONG,
                    0);  // interpollation dans la lecture du buffer pour �viter les clics
    class_addmethod(c, (method)liveGranul_clear, "clear",
                    0);  // panique ! effacement des grains en cours
    class_addmethod(c, (method)liveGranul_clear, "panic",
                    0);  // panique ! effacement des grains en cours

    class_addmethod(c, (method)liveGranul_multichanneloutputs, "multichanneloutputs", A_CANT,
                    0);  // mc Multichannel output
...
void liveGranul_grain(t_liveGranul* x, t_symbol* s, short ac, t_atom* av)
{
    ...
    if (ac < 8) {
        post(
            "livegranul~ : grain args are <decldelay(ms)> <delay> <detune> <amp> <length> <pan> "
            "<dist> <envbuffer>");
        return;
    }
        ...
        if (x->x_constr_type) {
            if (!x->x_freeze || x->x_freeze_pad) {
                if (x->Vtranspos[p] != 1.) {
                    if (x->Vtranspos[p] > 1.) {
                        ...
                        t_maxdelay = x->x_buf_frames - x->x_freeze_pad_curr +
                                     (x->Vtranspos[p] - 1.) * t_length2;
                    }
                }
            } else  /// if FREEZE
            {
                if (x->Vtranspos[p] > 0.) {
                    ...
                    t_maxdelay =
                        x->x_buf_frames - x->x_freeze_pad_curr + x->Vtranspos[p] * t_length2;
                }
            }
        } else {
            if (!x->x_freeze || x->x_freeze_pad) {
                if (x->Vtranspos[p] != 1.) {
                    if (x->Vtranspos[p] > 1.) {
                        ...
                        t_maxlength = -(x->x_buf_frames - t_delay - x->x_freeze_pad_curr) /
                                      (x->Vtranspos[p] - 1.);
                    }
                } else {
            ...
            } else  /// if FREEZE
            {
                if (x->Vtranspos[p] > 0.) {
                    ...
                    t_maxlength =
                        -(x->x_buf_frames - t_delay - x->x_freeze_pad_curr) / (x->Vtranspos[p]);
                }
            }
        }
...
void* liveGranul_new(t_symbol* s, short ac, t_atom* av)
{
    ...
    if (ac < 2) {
        ...
        for (j = 0; j < ac; j++) {
            switch (av[j].a_type) {
                case A_LONG:
                    //	post("argument %ld is a long : %ld, assigned to nb outs",
                    //(long)j,av[j].a_w.w_long);
                ...
                case A_FLOAT:
                    ...
                    post("liveGranul~ : set buffer size to %10.1f ms",
                         x->x_buf_frames * 1000. / x->x_sr);
...
// info outlet
        ...
        x->x_linear_interp_table =
            (t_linear_interp*)t_getbytes(TABLE_SIZE * sizeof(struct linear_interp));
        ...
        case 1:  // inlet
            switch (a - 1) {
                case -1:
                    ...
                    break;
                case 0:
                    sprintf(s, " bang ,set , signal zero X-ing, ...");
                    break;
                case 1:
                    sprintf(s, " delay (float & signal) ");
                    break;
                case 2:
...
{
    int i;
    if (!(x->x_ind = (long*)t_getbytes(NVOICES * sizeof(long)))) {
        return 0;
    }
    if (!(x->x_remain_ind = (long*)t_getbytes(NVOICES * sizeof(long)))) {
        return 0;
    }
    if (!(x->x_sind = (double*)t_getbytes(NVOICES * sizeof(double)))) {
        return 0;
    }

    if (!(x->Vbeg = (double*)t_getbytes(NVOICES * sizeof(double)))) {
        return 0;
    }
    if (!(x->Vtranspos = (double*)t_getbytes(NVOICES * sizeof(double)))) {
        return 0;
    }
    if (!(x->Vamp = (double*)t_getbytes(NVOICES * sizeof(double)))) {
        return 0;
    }
    if (!(x->Vlength = (double*)t_getbytes(NVOICES * sizeof(double)))) {
        return 0;
    }
    if (!(x->Vpan = (double*)t_getbytes(NVOICES * sizeof(double)))) {
        return 0;
    }
    if (!(x->Vdist = (double*)t_getbytes(NVOICES * sizeof(double)))) {
        return 0;
    }

    if (!(x->envinc = (double*)t_getbytes(NVOICES * sizeof(double)))) {
        return 0;
    }
    if (!(x->envind = (double*)t_getbytes(NVOICES * sizeof(double)))) {
        return 0;
    }
    if (!(x->x_env = (double*)t_getbytes(NVOICES * sizeof(double)))) {
        return 0;
    }
    if (!(x->Venv = (int*)t_getbytes(NVOICES * sizeof(int)))) {
        return 0;
    }

    if (!(x->x_delay = (int*)t_getbytes(NVOICES * sizeof(int)))) {
        return 0;
    }
    if (!(x->x_voiceOn = (int*)t_getbytes(NVOICES * sizeof(int)))) {
        return 0;
    }
...
int liveGranul_desalloc(t_liveGranul* x)
{
    ...
    if (x->x_sind) {
        t_freebytes(x->x_sind, NVOICES * sizeof(double));
    }

    if (x->Vbeg) {
        t_freebytes(x->Vbeg, NVOICES * sizeof(double));
    }
    if (x->Vtranspos) {
        t_freebytes(x->Vtranspos, NVOICES * sizeof(double));
    }
    if (x->Vamp) {
    ...
    if (x->w) {
        t_freebytes(x->w, (11 + x->x_nouts) * sizeof(double**));
    }
                          ...
                          void* userparam)
{
    ...
    // Meme chose avec declenchement par zero-crossing sur tout le vecteur in, dans la limite fixee
    // par nvoices
    while (n-- && p) {
        ...
        if (prevsigin <= 0 && sigin > 0)  // creation d'un grain
        {
            ...
            if (p) {
                ...
                t_length2 =
                    fabs((x->x_in5con) * t_length[xn] * srms + (1 - x->x_in5con) * lengthind);
                ...
                if (x->x_constr_type) {
                    if (!x->x_freeze || x->x_freeze_pad) {
                        if (x->Vtranspos[p] != 1.) {
                            if (x->Vtranspos[p] > 1.) {
                                t_mindelay =
                                    (x->Vtranspos[p] - 1.) * t_length2 - x->x_freeze_pad_curr;
                            } else {
                                t_maxdelay = t_bufframes - x->x_freeze_pad_curr +
                                             (x->Vtranspos[p] - 1.) * t_length2;
                            }
                        }
                    } else  /// if FREEZE
                    {
                        if (x->Vtranspos[p] > 0.) {
                            ...
                            t_maxdelay =
                                t_bufframes - x->x_freeze_pad_curr + x->Vtranspos[p] * t_length2;
                        }
                    }
                } else {
                    if (!x->x_freeze || x->x_freeze_pad) {
                        if (x->Vtranspos[p] != 1.) {
                            if (x->Vtranspos[p] > 1.) {
                                t_maxlength =
                                    (t_delay + x->x_freeze_pad_curr) / (x->Vtranspos[p] - 1.);
                            } else {
                                t_maxlength = -(t_bufframes - t_delay - x->x_freeze_pad_curr) /
                                              (x->Vtranspos[p] - 1.);
                            }
                        } else {
                            t_maxlength = 4646000;  // NOT MORE OK ???
                    ...
                    } else  /// if FREEZE
                    {
                        if (x->Vtranspos[p] > 0.) {
                            ...
                            t_maxlength =
                                -(t_bufframes - t_delay - x->x_freeze_pad_curr) / (x->Vtranspos[p]);
                        }
                    }
                }
                ...
                if (x->x_in5con) {
                    if (t_length[xn] < 0) {
                        ...
                        x->Vlength[p] = MIN(x->Vlength[p], t_maxlength);
                        x->envinc[p] =
                            -1. * (double)(x->x_env_frames[t_active_env] - 1.) / x->Vlength[p];
...
perform:
    ...
    for (i = 0; i < nvoices; i++) {
        ...
        if (x->x_voiceOn[i])  //&& x->x_ind[i] < x->Vlength[i] )
        {
            ...
            // grains) Il se peut que le buffer associ� au grain ne soit plus valide -> verification
            // -> si non on prend buffer #0
